...
object Aayush_Sinha_problem1 {
  def main(args: Array[String]) {

    val start_time = System.currentTimeMillis()
    val conf = new SparkConf().setAppName("Aayush_Sinha_Son").setMaster("local[2]")
    val sc = new SparkContext(conf)
    val csvfile = args(1)
    val csv = sc.textFile(csvfile,1)
    ...
    val outputfilename = "Aayush_Sinha_SON_"+csvfile.substring(csvfile.lastIndexOf("/")+1, csvfile.lastIndexOf("."))+".case"+ args(0) + ".txt"
